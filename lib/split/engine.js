...
import { computeItemizedSplit } from "./itemized.js";
 ...
 * Itemized split (M21).
 *
 * Each line item is assigned to whoever consumed it; a person's share is the
 * sum of their per-item portions. Delegates to lib/split/itemized.js, which
 * guarantees the parts sum to exactly the total.
 *
 * `values.items` carries the line items - the only method whose input is a
 * list rather than a per-participant map.
 */
export function splitItemized(total, participantIds, values) {
  const ids = assertParticipants(participantIds);
  const value = assertPositiveTotal(total);
  ...
  try {
    ...
    // Surface as a SplitError so every method fails the same way.
    throw new SplitError(error.message, "INVALID_ITEMS");
  }
}
