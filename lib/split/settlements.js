...
/**
 * What a settlement does to a pairwise balance, from `meId`'s point of view.
 * Mirrors the sign convention in balances.js: paying someone reduces what you
 * owe, so your balance moves toward zero.
 */
export function applySettlement(netBalance, { fromUserId }, meId) {
  const net = toDecimal(netBalance ?? 0);
  return fromUserId === meId ? net : net;
}
