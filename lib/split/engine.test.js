...
describe("splitItemized", () => {
  test("splits by who consumed what", () => {
    const splits = splitItemized(300, THREE, {
      items: [
        { name: "Pizza", amount: "200.00", assignedTo: [AYUSH, RAHUL] },
        { name: "Juice", amount: "100.00", assignedTo: [PRIYA] },
      ],
    });
    ...
    assert.equal(byId[AYUSH], "100.00");
    assert.equal(byId[RAHUL], "100.00");
    assert.equal(byId[PRIYA], "100.00");
    assertExact(splits, 300, "itemized");
  });

  test("items that do not add up to the total are rejected", () => {
    assert.throws(
      () =>
        splitItemized(500, THREE, {
          items: [{ name: "Pizza", amount: "200.00", assignedTo: [AYUSH] }],
        }),
      (e) => e instanceof SplitError && /under the total/.test(e.message)
    );
  });

  test("errors surface as SplitError like every other method", () => {
    assert.throws(
      () => splitItemized(100, THREE, { items: [] }),
      (e) => e instanceof SplitError && e.code === "INVALID_ITEMS"
    );
  });

  test("missing items are rejected, not silently treated as empty", () => {
    assert.throws(() => splitItemized(100, THREE, {}), SplitError);
  });
});
...
describe("computeSplit dispatcher", () => {
  ...
  test("every declared method is routed", () => {
    for (const method of SPLIT_METHODS) {
      ...
      if (method === "ITEMIZED") {
        // Routed, but these values carry no line items, so it rejects.
        assert.throws(call, (e) => e.code === "INVALID_ITEMS");
