...
describe("contributionsBetween - traceability", () => {
  const ledger = {
    expenses: [
      {
        id: "e1",
      ...
      },
      {
        ...
        amount: "300.00",
        isDeleted: false,
        splits: [{ userId: AYUSH, shareAmount: "300.00" }],
      },
    ],
    settlements: [
      {
  ...
  test("returns only rows involving both people", () => {
    const rows = contributionsBetween(ledger, AYUSH, RAHUL);
    assert.deepEqual(rows.map((r) => r.id).sort(), ["e1", "e2", "s1"]);
  });

  test("excludes a third party's rows", () => {
    const rows = contributionsBetween(ledger, AYUSH, PRIYA);
    assert.deepEqual(rows.map((r) => r.id), ["e1"]);
  });

  test("signs follow the pairwise convention", () => {
    const rows = contributionsBetween(ledger, AYUSH, RAHUL);
    const byId = Object.fromEntries(rows.map((r) => [r.id, r.contribution]));

    assert.equal(fixed(byId.e1), "1000.00", "I paid, they owe their share");
    assert.equal(fixed(byId.e2), "-300.00", "they paid, I owe my share");
    assert.equal(fixed(byId.s1), "-200.00", "they paid me back");
  });

  test("contributions sum to exactly the pairwise balance", () => {
    for (const other of [RAHUL, PRIYA]) {
      const rows = contributionsBetween(ledger, AYUSH, other);
      const traced = sum(rows.map((r) => r.contribution));
      const pairwise = pairwiseForUser(ledger, AYUSH).get(other) ?? new Decimal(0);

      assert.equal(
        fixed(traced),
        fixed(pairwise),
        `every rupee of the ${other} balance must be traceable`
      );
    }
  });

  test("the payer's own share is not a debt to themselves", () => {
    const rows = contributionsBetween(ledger, AYUSH, RAHUL);
    // e1's 1000 is Rahul's share, not Ayush's own 1000.
    assert.equal(fixed(rows.find((r) => r.id === "e1").share), "1000.00");
  });

  test("zero shares are skipped", () => {
    const rows = contributionsBetween(
      {
        expenses: [
          {
            id: "z",
            ...
            isDeleted: false,
            splits: [
              { userId: AYUSH, shareAmount: "100.00" },
              { userId: RAHUL, shareAmount: "0.00" },
            ],
          },
        ],
      },
  ...
  test("deleted expenses contribute nothing", () => {
    const rows = contributionsBetween(
      { expenses: [{ ...ledger.expenses[0], isDeleted: true }] },
      AYUSH,
      RAHUL
    );
    assert.equal(rows.length, 0);
  ...
  test("an expense where neither is the payer is excluded", () => {
    const rows = contributionsBetween(
      {
        expenses: [
          {
    ...
    assert.equal(rows.length, 0, "Priya paid, so this is not an Ayush/Rahul debt");
  });
  ...
  test("traceability holds across random ledgers", () => {
    let seed = 777001;
    const rand = () => {
      seed = (seed * 1664525 + 1013904223) % 4294967296;
      return seed / 4294967296;
    ...
    for (let i = 0; i < 200; i++) {
      ...
      for (let e = 0; e < 1 + Math.floor(rand() * 4); e++) {
        const amt = new Decimal(1 + Math.floor(rand() * 50000)).div(100);
        const count = 1 + Math.floor(rand() * people.length);
        const participants = people.slice(0, count);
        const shares = allocate(amt, participants.map(() => 1));
        expenses.push({
          id: `e${e}`,
          paidById: people[Math.floor(rand() * people.length)],
          amount: amt,
      ...
      for (let k = 0; k < Math.floor(rand() * 3); k++) {
        ...
        settlements.push({
          ...
          fromUserId: from,
          toUserId: to,
          amount: new Decimal(Math.floor(rand() * 5000)).div(100),
        });
      }

      const ledgerCase = { expenses, settlements };
      const pw = pairwiseForUser(ledgerCase, AYUSH);

      for (const other of people.filter((p) => p !== AYUSH)) {
        const traced = sum(
          contributionsBetween(ledgerCase, AYUSH, other).map((r) => r.contribution)
        );
        const expected = pw.get(other) ?? new Decimal(0);
        assert.equal(
          traced.toFixed(2),
          expected.toFixed(2),
          `case ${i}: ${other} balance not fully traceable`
        );
      }
    }
  });
